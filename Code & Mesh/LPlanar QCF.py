...
def allTimeSteps(runIndx=0, paramIndex=0, startingInd=0):
    ...
    if paramterType=="l-conformal":
        a=0; b=1; c=0;d=0;e=0;  regC=100000; regL = 1000000;
    elif paramterType == "l-elastic":
        a=1; b=1;  c=0;d=0;e=0; regC=100000; regL = 1000000; 
    elif paramterType=="l-uniform":
        a=0; b=0; c=0.1;d=0.1;e=0.0; regC=100000; regL = 1000000;
    else: 
    ...
    for stepNum in range(startingInd,numTimeSteps):
        ...
        newVerts, initStrains , finalStrains, maxDeformation = stepInTime(stepNum, runIndx,
                             paramIndex, a, b, c, d, e, regC, regL)
...
#===========================================================================================================
def stepInTime(stepNum, runIndx, paramIndex, a, b, c, d, e, regC, regL):
    '''
    '''
    ...
    vertices1 = onp.loadtxt(loadDir / "vertices_deformed.csv", delimiter=",", dtype=np.float64)
    ...
    landmarkInds = onp.loadtxt(meshDir / runName / "landmarks.csv", delimiter=",", dtype=np.int32)
    ...
    landmarkDisps = displacements[landmarkInds]
    ...
    res = solver.run(displacements, startingVerts=vertices0, initBoundaryDisps=initBoundaryDisps, 
                     faces=faces, areaMat=areaMat, boundaryInds=boundaryInds, landmarkInds=landmarkInds,
                      landmarkDisps=landmarkDisps, normals=boundaryNormals, gradCostMat=gradCostMat, 
                      boundaryLengths=boundaryLengths, prevStrains=previousStrains, 
                initialArea=initialArea, duration=duration, a=a, b=b, c=c,  d=d, e=E, regC=regC, regL=regL)
...
#===========================================================================================================
@jit
def cost(disps, startingVerts, initBoundaryDisps, faces, areaMat, boundaryInds,
          landmarkInds, landmarkDisps, normals, gradCostMat, boundaryLengths, prevStrains, 
          initialArea, duration, a, b, c, d, e, regC, regL) : 
    '''
    '''
    ...
    #enforce landmark displacements.   
    cost += (duration/initialArea) * regL*((disps[landmarkInds] - landmarkDisps )**2).sum()
